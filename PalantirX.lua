--[[ SYNTHESISED BUNDLE: vonlib.lua (YoungSlaves/Libary) + main.lua features ]]
...
--[[ ==================== FEATURES (main.lua) ==================== ]]
--[[
	Palantir-style feature set, built on the YoungSlaves/Libary GUI (vonlib.lua / VonLib).

	REQUIRES:
		- vonlib.lua from https://github.com/YoungSlaves/Libary
		  It is auto-loaded from raw.githubusercontent at runtime if not already present.
		  If your executor blocks game:HttpGet, paste the contents of vonlib.lua above this file
		  and leave the "already loaded" branch to pick it up.

	WHAT THIS BUILDS (no game features ship in the repo, so all of this is ours):
		[Visuals]
			- Box + name + health-bar + distance ESP with team filter, colors, refresh rate.
		[Movement]
			- Speedhack    (boost move speed along your input direction, velocity-capped)
			- Noclip       (client-side collision strip, optional knock-only mode)
			- Flight       (camera-relative WASD + Space/Ctrl free flight)
		[Movement Bypass layer]
			- Velocity resolver: after every artificial CFrame move we set a sane linear/angular
			  velocity so the server's velocity sanity-check sees a believable motion vector.
			- Sub-step smoothing: moves are applied as small clamped lerp steps per frame instead
			  of single teleports (big CFrame deltas are what trip server re-sync / rubber-band).
			- Ground glue: the root is raycast-glued to the floor so you don't "float" (hovering
			  is a classic movement-check red flag).
			- Speed cap + max-step cap, all tunable.

	DISCLAIMER: these are client-side movement techniques. Every game validates the client
	differently (Deepwoken's server re-sync, velocity checks, Verity, whitelists). There is no
	universal detection-safe bypass; the "bypass" layer exists to make the motion look legitimate,
	but it must be tuned per server and absolutely cannot guarantee evasion.
--]]

local VONLIB_URL = "https://raw.githubusercontent.com/YoungSlaves/Libary/main/vonlib.lua"

-- ---------------- VonLib loader ------------------------------------------
local VonLib = rawget(_G, "VonLib")
if not VonLib then
	local env = getrenv and getrenv() or _G
	VonLib = rawget(env, "VonLib")
end
if not VonLib then
	local ok, src = pcall(function()
		return game:HttpGet(VONLIB_URL)
	end)
	if ok and src then
...
if not VonLib then
	error("[PalantirX] Unable to load VonLib UI (raw.githubusercontent unreachable). Paste vonlib.lua above this file.")
end
...
local espRenderConn  -- RenderStepped connection for ESP, so Unload can disconnect it
...
local DEFAULT_LEGIT_SPEED = 24      -- Deepwoken run ≈ this; velocity cap target
local DEFAULT_STEP_MAX = 3.0        -- max studs allowed in a single lerp step

-- ---------------- Shared option state derived from the GUI ---------------
local Options = {
	SpeedCap = DEFAULT_LEGIT_SPEED,
	MaxStep = DEFAULT_STEP_MAX,
	GroundGlue = true,
	VelocityResolve = true,
	AllyFiltered = true,   -- treat same-team players as allies
}

local function IsAlly(p)
	if p == LocalPlayer then return true end
	return p.Team ~= nil and LocalPlayer.Team ~= nil
		and p.Team == LocalPlayer.Team
		and Options.AllyFiltered
end

-- ---------------- Movement bypass helpers --------------------------------
local GroundRayParams = RaycastParams.new()
GroundRayParams.FilterType = Enum.RaycastFilterType.Exclude

local function GroundGlue(hrp, dt)
	if not Options.GroundGlue then return end
	...
	if ray then
		local targetY = ray.Position.Y + 0.4
		local dY = targetY - pos.Y
		if dY > 1e-3 then
			hrp.CFrame = hrp.CFrame * CFrame.new(0, math.min(dY, Options.MaxStep), 0)
		end
	end
end

local function ResolveVelocity(hrp, prev, dt, motiveSpeed)
	if not Options.VelocityResolve then return end
	if (not prev) or dt <= 0 then
	...
	local v = delta / dt
	local cap = motiveSpeed or Options.SpeedCap
	if v.Magnitude > cap then v = v.Unit * cap end
	hrp.AssemblyLinearVelocity = v
	hrp.AssemblyAngularVelocity = Vector3.zero
end

local function SafeLerpToward(targetCF, moveSpeed, dt, maxStep, noGlue)
	local c, hrp, hum = GetRoot()
	if not hrp then return end
	maxStep = maxStep or Options.MaxStep
	dt = dt or (1 / 60)

	local start = hrp.CFrame
	local startPos, targetPos = start.Position, targetCF.Position
	local dist = (targetPos - startPos).Magnitude
	local step = math.min(dist, moveSpeed * dt, maxStep)
	if step <= 1e-3 then
		ResolveVelocity(hrp, startPos, dt)
		return
	end
	local lerpK = math.max(0, math.min(1, step / math.max(1e-4, dist)))
	local next = start:Lerp(targetCF, lerpK)
	hrp.CFrame = next
	-- "noGlue" (flight) skips the floor-pull so the root can actually gain altitude
	if not noGlue then
		GroundGlue(hrp, dt)
	end
	ResolveVelocity(hrp, startPos, dt, moveSpeed)
end

-- ---------------- Generic toggle-managed loop ----------------------------
local Features = {}

local function StopLoop(name)
	local f = Features[name]
	if f and f.thread then
		f.running = false
		task.spawn(function()
			if f.thread then
				task.cancel(f.thread)
			end
		end)
		f.thread = nil
	end
...
local function StartLoop(name, fn)
	...
	f.thread = task.spawn(function()
		while f.running do
			...
			if not ok then warn("[PalantirX:" .. name .. "] ", err) end
			task.wait()
		end
	end)
end
...
local function ESPWorldPosition(p)
	local c = p.Character
	if not c then return nil end
	local hrp = c:FindFirstChild("HumanoidRootPart")
	local head = c:FindFirstChild("Head")
	if not hrp then return nil end
	if head then return hrp.Position, head.Position end
	return hrp.Position, hrp.Position + Vector3.new(0, 3, 0)
end

local function ESPEnsure(p)
...
local function ESPLoop()
	local now = os.clock()
	local rr = ESP.Refresh
	if ESP._last and (now - ESP._last) < rr then return end
	ESP._last = now

	local _, lpHRP = GetRoot()
	for p, d in pairs(ESP.Drawings) do
		local keep = false
		if ESP.Enabled and p.Parent and p ~= LocalPlayer then
			local eyePos, headPos = ESPWorldPosition(p)
			local hrp = p.Character and p.Character:FindFirstChild("HumanoidRootPart")
			local hum = p.Character and p.Character:FindFirstChildOfClass("Humanoid")
			if eyePos and headPos and hrp and hum and hum.Health > 0 then
				local onScreen, sp, depth = Camera:WorldToViewportPoint(eyePos)
				local onTop, tp = Camera:WorldToViewportPoint(headPos + Vector3.new(0, 0.6, 0))
				if onScreen and depth > 0 then
					keep = true
					local color = IsAlly(p) and ESP.TeamColor or ESP.BoxColor
					local halfW = math.max(2, (tp.Y - sp.Y) * 0.55)
					local topLeft = Vector2.new(sp.X - halfW, sp.Y)
					local botRight = Vector2.new(sp.X + halfW, tp.Y)

					if not d.box.Visible or d.box.Color ~= color then
						d.box.Color = color
						d.box.Visible = true
					end
					d.box.Position = topLeft
					d.box.Size = botRight - topLeft

					local hpRatio = 1
					if hum and hum.MaxHealth > 0 then
						hpRatio = math.clamp(hum.Health / hum.MaxHealth, 0, 1)
					end
					local barY = topLeft.Y - 5
					...
					d.health.From = Vector2.new(sp.X, barY)
					d.health.To = Vector2.new(sp.X, barY - 34 * hpRatio)

					local dist = lpHRP and math.round((hrp.Position - lpHRP.Position).Magnitude) or 0
					d.name.Visible = true
					d.name.Color = color
					d.name.Text = p.Name .. "  [" .. dist .. "]"
					d.name.Position = Vector2.new(topLeft.X, topLeft.Y - 16)
				end
			end
		end
		for _, o in pairs(d) do
			if not keep then o.Visible = false end
		end
	end
...
-- =========================================================================
--  MOVEMENT — Speedhack / Noclip / Flight
-- =========================================================================
local MovementExt = {
	Speedhack = false,
	SpeedMultiplier = 1.5,       -- x normal run speed
	SpeedMaxVel = 40,            -- velocity cap reported to the server while boosting
	Noclip = false,
	NoclipKnockedOnly = false,   -- only strip collision while knocked/ragdolled
	Flight = false,
	FlightSpeed = 40,
...
local function Collisions(enable)
	local c = GetRoot()
...
-- Speedhack: nudge the root along the intended movement direction at a boosted
-- rate, then let ResolveVelocity (inside SafeLerpToward) report a capped velocity.
local SpeedhackFn = function()
	local c, hrp, hum = GetRoot()
	if not hrp or not hum then return end
	local dir = hum.MoveDirection
	if dir.Magnitude < 0.05 then return end

	local boost = Options.SpeedCap * MovementExt.SpeedMultiplier
	local cap = math.min(math.max(boost, 0), MovementExt.SpeedMaxVel)
	local localDir = hrp.CFrame:VectorToObjectSpace(dir)
	local goal = hrp.CFrame * CFrame.new(localDir * (cap * (1 / 60)))
...
local NoclipFn = function()
	...
	local knocked = st == Enum.HumanoidStateType.Ragdoll or st == Enum.HumanoidStateType.KnockedDown
	if MovementExt.NoclipKnockedOnly and not knocked then
		hrp.CanCollide = true
		return
	end
...
local FlightFn = function()
	...
	if UIS:IsKeyDown(Enum.KeyCode.W) then move = move + Vector3.new(fwd.X, 0, fwd.Z) end
...
-- =========================================================================
--  GUI — build the VonLib window
-- =========================================================================
local Window = VonLib:Window({
	scriptname = "PalantirX",
	destroyifexists = true,
	theme = "Von",
})

-- ---------------------------------------------------------------- VISUALS
local VisualsTab = Window:Tab("Visuals", "Visuals")
VisualsTab:Section("ESP")

VisualsTab:Toggle("ESP", "Box + health + name + distance ESP", false, function(v)
	ESP.Enabled = v
	if not v then
...
VisualsTab:Toggle("Filter Teammates", "Draw teammates with team color", true, function(v)
	ESP.Team = v
	Options.AllyFiltered = v
end)

VisualsTab:ColorPicker("Enemy Color", "Color for enemies", ESP.BoxColor, function(c)
	ESP.BoxColor = c
end)

VisualsTab:ColorPicker("Team Color", "Color for teammates", ESP.TeamColor, function(c)
	ESP.TeamColor = c
end)

VisualsTab:Slider("Refresh Rate", "Seconds per ESP update", 0.03, 0.5, function(v)
	ESP.Refresh = v
end):SetValue(0.08)

-- Redraw loop always runs; it just toggles visibility off when ESP.Enabled == false
espRenderConn = RunService.RenderStepped:Connect(function()
	pcall(ESPLoop)
end)

-- ------------------------------------------------------------- MOVEMENT
local MoveTab = Window:Tab("Movement", "Movement")

MoveTab:Section("Speedhack")
Features.Speedhack = {}
MoveTab:Toggle("Speedhack", "Boost move speed along your input direction", false, function(v)
	MovementExt.Speedhack = v
	if v then StartLoop("Speedhack", SpeedhackFn) else StopLoop("Speedhack") end
end)
MoveTab:Slider("Speed Multiplier", "x normal run speed", 1.1, 4, function(v)
	MovementExt.SpeedMultiplier = v
end):SetValue(1.5)
MoveTab:Slider("Velocity Cap", "Max velocity the resolver reports while boosting", 10, 80, function(v)
	MovementExt.SpeedMaxVel = v
end):SetValue(40)
...
MoveTab:Section("Movement Bypass (very important)")
MoveTab:Slider("Legit Speed Cap", "Velocity resolver cap (believeable walk/run speed)", 8, 40, function(v)
	Options.SpeedCap = v
end):SetValue(DEFAULT_LEGIT_SPEED)

MoveTab:Slider("Max Per-Frame Step", "Max studs per frame change (bigger = more detectable)", 0.5, 6, function(v)
	Options.MaxStep = v
end):SetValue(DEFAULT_STEP_MAX)

MoveTab:Toggle("Ground Glue", "Raycast-glue to floor to avoid floating", true, function(v)
	Options.GroundGlue = v
end)

MoveTab:Toggle("Velocity Resolver", "Set sane velocity after artificial moves", true, function(v)
	Options.VelocityResolve = v
end)

-- ---------------------------------------------------------------- CONFIG
-- Unload: stop every feature loop, drop the ESP drawings + connection, restore
-- the character, and destroy the GUI. Safe to call from the button or a keybind.
local function Unload()
	for name in pairs(Features) do
		StopLoop(name)
	end
	if espRenderConn then
		pcall(espRenderConn.Disconnect, espRenderConn)
		espRenderConn = nil
	end
	for _, d in pairs(ESP.Drawings) do
		for _, o in pairs(d) do
	...
	warn("[PalantirX] Unloaded.")
end

local ConfigTab = Window:Tab("Config", "Config")
ConfigTab:Button("Save Config", "Writes current settings to Von folder", function()
	VonLib:NewConfig("PalantirX")
	VonLib:SaveConfig()
end)
ConfigTab:Button("Unload Script", "Stop all features and close the GUI", Unload)

VonLib:AddNotifications()

warn("[PalantirX] Loaded. Built on YoungSlaves/Libary (VonLib).")